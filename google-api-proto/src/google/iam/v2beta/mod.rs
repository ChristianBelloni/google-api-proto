...
pub mod policies_server {
    ...
    #[async_trait]
    pub trait Policies: std::marker::Send + std::marker::Sync + 'static {
        /// Retrieves the policies of the specified kind that are attached to a
        /// resource.
        ///
        /// The response lists only policy metadata. In particular, policy rules are
    ...
    impl<T> Clone for PoliciesServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "google.iam.v2beta.Policies";
    impl<T> tonic::server::NamedService for PoliciesServer<T> {
