...
pub mod source_repo_server {
    ...
    #[async_trait]
    pub trait SourceRepo: std::marker::Send + std::marker::Sync + 'static {
        /// Returns all repos belonging to a project. The sizes of the repos are
        /// not set by ListRepos.  To get the size of a repo, use GetRepo.
        async fn list_repos(
            &self,
    ...
    impl<T> Clone for SourceRepoServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "google.devtools.sourcerepo.v1.SourceRepo";
    impl<T> tonic::server::NamedService for SourceRepoServer<T> {
