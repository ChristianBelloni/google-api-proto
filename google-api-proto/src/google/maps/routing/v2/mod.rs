...
pub mod routes_server {
    #![allow(unused_variables, dead_code, missing_docs, clippy::let_unit_value)]
    use tonic::codegen::*;
    /// Generated trait containing gRPC methods that should be implemented for use with RoutesServer.
    #[async_trait]
    pub trait Routes: std::marker::Send + std::marker::Sync + 'static {
        /// Returns the primary route along with optional alternate routes, given a set
        /// of terminal and intermediate waypoints.
        ///
        /// **NOTE:** This method requires that you specify a response field mask in
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for RoutesServer<T>
    where
        T: Routes,
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/google.maps.routing.v2.Routes/ComputeRoutes" => {
                    ...
                    struct ComputeRoutesSvc<T: Routes>(pub Arc<T>);
                    impl<
                        T: Routes,
                    ...
                    for ComputeRoutesSvc<T> {
                        ...
                        fn call(
                            ...
                            let fut = async move {
                                <T as Routes>::compute_routes(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
                ...
                "/google.maps.routing.v2.Routes/ComputeRouteMatrix" => {
                    #[allow(non_camel_case_types)]
                    struct ComputeRouteMatrixSvc<T: Routes>(pub Arc<T>);
                    impl<
                        T: Routes,
                    ...
                    > for ComputeRouteMatrixSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as Routes>::compute_route_matrix(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    impl<T> Clone for RoutesServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "google.maps.routing.v2.Routes";
    impl<T> tonic::server::NamedService for RoutesServer<T> {
