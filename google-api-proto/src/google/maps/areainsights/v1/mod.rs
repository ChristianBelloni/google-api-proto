...
pub mod area_insights_server {
    #![allow(unused_variables, dead_code, missing_docs, clippy::let_unit_value)]
    use tonic::codegen::*;
    /// Generated trait containing gRPC methods that should be implemented for use with AreaInsightsServer.
    #[async_trait]
    pub trait AreaInsights: std::marker::Send + std::marker::Sync + 'static {
        /// Compute Insights RPC
        ///
        /// This method lets you retrieve insights about areas using a variaty of
        /// filter such as: area, place type, operating status, price level
        ...
        async fn compute_insights(
            &self,
            request: tonic::Request<super::ComputeInsightsRequest>,
        ) -> std::result::Result<
            tonic::Response<super::ComputeInsightsResponse>,
            tonic::Status,
        >;
    ...
    /// Service definition for the Area Insights API.
    #[derive(Debug)]
    pub struct AreaInsightsServer<T> {
        inner: Arc<T>,
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for AreaInsightsServer<T>
    where
        T: AreaInsights,
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/google.maps.areainsights.v1.AreaInsights/ComputeInsights" => {
                    ...
                    struct ComputeInsightsSvc<T: AreaInsights>(pub Arc<T>);
                    impl<
                        T: AreaInsights,
                    > tonic::server::UnaryService<super::ComputeInsightsRequest>
                    for ComputeInsightsSvc<T> {
                        type Response = super::ComputeInsightsResponse;
                        ...
                        fn call(
                            ...
                            let fut = async move {
                                <T as AreaInsights>::compute_insights(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
                    ...
                    let fut = async move {
                        let method = ComputeInsightsSvc(inner);
                        let codec = tonic::codec::ProstCodec::default();
    ...
    impl<T> Clone for AreaInsightsServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "google.maps.areainsights.v1.AreaInsights";
    impl<T> tonic::server::NamedService for AreaInsightsServer<T> {
