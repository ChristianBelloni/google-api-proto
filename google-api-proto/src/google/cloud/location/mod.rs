...
pub mod locations_server {
    ...
    #[async_trait]
    pub trait Locations: std::marker::Send + std::marker::Sync + 'static {
        /// Lists information about the supported locations for this service.
    ...
    impl<T> Clone for LocationsServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "google.cloud.location.Locations";
    impl<T> tonic::server::NamedService for LocationsServer<T> {
