...
pub mod video_stitcher_service_server {
    ...
    #[async_trait]
    pub trait VideoStitcherService: std::marker::Send + std::marker::Sync + 'static {
        /// Creates a new CDN key.
    ...
    /// content with any standard VMAP compliant ad server.
    #[derive(Debug)]
    pub struct VideoStitcherServiceServer<T> {
        inner: Arc<T>,
    ...
    impl<T> VideoStitcherServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T> Clone for VideoStitcherServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "google.cloud.video.stitcher.v1.VideoStitcherService";
    impl<T> tonic::server::NamedService for VideoStitcherServiceServer<T> {
