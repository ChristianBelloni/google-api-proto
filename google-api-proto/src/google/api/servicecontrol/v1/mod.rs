...
pub mod quota_controller_server {
    #![allow(unused_variables, dead_code, missing_docs, clippy::let_unit_value)]
    use tonic::codegen::*;
    /// Generated trait containing gRPC methods that should be implemented for use with QuotaControllerServer.
    #[async_trait]
    pub trait QuotaController: std::marker::Send + std::marker::Sync + 'static {
        /// Attempts to allocate quota for the specified consumer. It should be called
        /// before the operation is executed.
        ///
        /// This method requires the `servicemanagement.services.quota`
    ...
    /// service](https://cloud.google.com/service-management/reference/rpc/google.api/servicemanagement.v1#google.api.servicemanagement.v1.ManagedService).
    #[derive(Debug)]
    pub struct QuotaControllerServer<T> {
        inner: Arc<T>,
    ...
    impl<T> QuotaControllerServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for QuotaControllerServer<T>
    where
        T: QuotaController,
                ...
                "/google.api.servicecontrol.v1.QuotaController/AllocateQuota" => {
                    ...
                    for AllocateQuotaSvc<T> {
                        ...
                        fn call(
                            ...
                            let fut = async move {
                                <T as QuotaController>::allocate_quota(&inner, request)
                                    .await
                            };
                            Box::pin(fut)
                        }
    ...
    impl<T> Clone for QuotaControllerServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "google.api.servicecontrol.v1.QuotaController";
    impl<T> tonic::server::NamedService for QuotaControllerServer<T> {
...
pub mod service_controller_server {
    ...
    /// Generated gRPC service name
    pub const SERVICE_NAME: &str = "google.api.servicecontrol.v1.ServiceController";
